module BulkTimeEntriesHelper
  ...
  def label_for_field(field, rnd_id, options = { })
    ...
    label_text += content_tag("span", " *", :class => "required") if options.delete(:required)
    label = content_tag("label", label_text.html_safe, 
  ...
  def get_issue_desc(issue_id)
    return "project (#{issue_id.blank? ? "" : Issue.find(issue_id)})";
  end
end
